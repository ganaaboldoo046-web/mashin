...
Also redirect `temmun-car.pages.dev` to the custom domain in the Cloudflare Pages custom-domain settings. Until that redirect is enabled, the project sends `X-Robots-Tag: noindex, nofollow` on the Pages subdomain.
...
- Cloudflare → Workers & Pages → `temmun-car` → Settings → Variables and Secrets → Production variable: `GOOGLE_CLIENT_ID`
