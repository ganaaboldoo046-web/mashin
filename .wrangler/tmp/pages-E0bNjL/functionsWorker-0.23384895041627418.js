...
async function onRequestGet(context) {
  ...
  const revenueFormatted = (revenueMNT / 1e6).toFixed(1) + "M \u20AE";
  const usersResult = await db.prepare(
    "SELECT COUNT(*) as count FROM users"
  ).first();
  const totalVisits = usersResult.count;
...
var onRequestGet2 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({
      rate: result ? parseFloat(result.value) : 2.5
    }), {
...
var onRequestPost = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    await env.DB.prepare("INSERT INTO settings (key, value, updated_at) VALUES ('exchange_rate', ?, strftime('%s', 'now')) ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at").bind(String(rate)).run();
    const updateQuery = `
            UPDATE products 
...
var onRequestPost2 = /* @__PURE__ */ __name(async (context) => {
  try {
    ...
    ).bind(body.productId, body.productName, body.userName, body.phone, body.facebookId || "", body.userId || null).run();
    if (success) {
...
// api/reviews_create.ts
var onRequestPost4 = /* @__PURE__ */ __name(async (context) => {
  try {
    const { request, env } = context;
    const body = await request.json();
    const { user_id, user_name, car_model, comment, rating, gender } = body;
    if (!user_id || !comment) {
    ...
    ).bind(user_id, user_name, car_model, comment, rating, gender || "male").run();
    if (success) {
      return new Response(JSON.stringify({ success: true }), {
        headers: { "Content-Type": "application/json" }
      });
...
async function onRequest7(context) {
  ...
  try {
    ...
    const statements = ids.map(
      (id, index) => db.prepare("UPDATE categories SET sort_order = ? WHERE id = ?").bind(index, id)
    );
...
// ../.wrangler/tmp/pages-E0bNjL/functionsRoutes-0.39914975059932367.mjs
var routes = [
  {
  ...
  {
    routePath: "/api/reservations_delete",
    mountPath: "/api",
    method: "DELETE",
    middlewares: [],
